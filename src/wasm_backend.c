...
static void vb_push_quad_gl(VertexBuf* self, int dl, int dt, int sl, int st, int w, int h) {
    int pdr = self->prev_dr;
    int pdt = self->prev_dt;
    int dr = dl + w;
    int db = dt + h;

    if (dl != pdr + 1 || dt != pdt) {
        vb_push(self, pdr, pdt, 0, 1);
        vb_push(self, pdr, pdt, 1, 0);
        vb_push(self, dl, db, 1, 1);
        vb_push(self, dl, db, 1, 0);
    }

    vb_push_quad(self, dl, dt, sl, st, w, h);
    self->prev_dr = dr;
    self->prev_dt = dt;
}
...
void be_blit_tile(Backend* be, int x, int y, int n) {
    ...
    if (be->gl) {
        vb_push_quad_gl(&be->sprites, x, y, sx, sy, TILE_W, TILE_H);
    } else {
...
void be_blit_text(Backend* be, int x, int y, char* str) {
    while (*str) {
        ...
        if (be->gl) {
            vb_push_quad_gl(&be->sprites, x, y, sx, sy, FONT_W, FONT_H);
        } else {
